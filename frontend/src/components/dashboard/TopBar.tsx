...
export const Topbar = () => {
  ...
  const avatarBgColor = getAvatarColor(user?.full_name || "User");
  ...
  const handleNotificationClick = (id: number, is_read: boolean) => {
    if (!is_read) {
  ...
  notifications.map((n) => (
    <div
      ...
      className={`px-5 py-4 cursor-pointer transition-all duration-200 flex gap-4 items-start
        ${!n.is_read ? "bg-gray-50 hover:bg-gray-100" : "hover:bg-gray-50"}
      `}
    >
      {/* CONTENT */}
