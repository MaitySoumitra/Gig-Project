...
export const DashBoardBody = () => {
  ...
  const taskStatus = (t: Task) => {
    const colId = typeof t.column === "object" ? t.column?.id : t.column;
    return currentColumns.find((c) => c.id === colId)?.name || null;
  };
      ...
      >
        {currentColumns.map((c) => {
          ...
          const tasksInColumn = task
            .filter((t) => {
              const taskColumnId =
                typeof t.column === "object" ? t.column?.id : t.column;
              return taskColumnId?.toString() === c.id?.toString();
            })
          ...
          return (
            <div
              ...
              onDrop={(e) => {
                ...
                moveTask(data.taskId, c.id, tasksInColumn.length);
              }}
            >
              {/* COLUMN HEADER */}
