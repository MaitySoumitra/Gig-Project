            ...
            <div className="bg-white w-full max-w-6xl max-h-[92vh] overflow-hidden flex flex-col rounded-2xl shadow-2xl border border-gray-100">
                ...
                <div className="flex-grow grid md:grid-cols-3 divide-x divide-gray-100 overflow-hidden">
                                    ...
                                    <div className="space-y-3 w-full">
                                        ...
                                        <UserSearchInput
                                            onUserSelect={(selectedUser) => {
                                                ...
                                                if (!current.some(u => u.id === selectedUser.id)) {
                            ...
                            <EditableRow
                                ...
                                editComponent={
                                    <div className="flex gap-2">
                                        <input type="date" min={formatToISODate(editedTask.start_date) || today} className="text-xs border p-1 rounded" value={formatToISODate(editedTask.start_date)} onChange={(e) => handleChange('start_date', e.target.value)} />
                                        <input type="date" min={formatToISODate(editedTask.due_date) || today} className="text-xs border p-1 rounded" value={formatToISODate(editedTask.due_date)} onChange={(e) => handleChange('due_date', e.target.value)} />
                                    </div>
                                }
                            >
                                <div className="text-sm font-medium text-gray-700">
                                    {formatDate(editedTask.start_date)} <span className="text-gray-300 mx-1">→</span> <span className="text-emerald-600">{formatDate(editedTask.due_date)}</span>
                                </div>
                            </EditableRow>
                    ...
                    <div className="bg-gray-50/30 h-full overflow-hidden">
                        <ActivityDetails taskId={editedTask.id} />
                    </div>
