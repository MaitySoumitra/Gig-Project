...
export const BoardProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getTaskByColumn = (columnId: string) => {
    return tasks.filter((t) => {
      const tColId = typeof t.column === "object" ? (t.column as any).id : t.column;
      const isRightColumn = tColId?.toString() === columnId.toString();
      const tBoardId = typeof t.board === "object" ? (t.board as any).id : t.board;
      const isRightBoard = tBoardId?.toString() === currentBoard?.id?.toString();
  ...
  const contextValue: BoardProviderProps = useMemo(
    () => ({
      ...
      addMember: (memberId: string) =>
        currentBoard && dispatch(addMember({ boardId: currentBoard.id, memberId })),
      addColumn: (name: string) =>
        currentBoard && dispatch(addColumn({ boardId: currentBoard.id, name })),
      deleteColumn: (columnId: string) =>
        currentBoard && dispatch(deleteColumn({ boardId: currentBoard.id, columnId })),
